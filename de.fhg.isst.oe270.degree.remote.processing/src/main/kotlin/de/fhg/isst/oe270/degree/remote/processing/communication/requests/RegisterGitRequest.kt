...
import de.fhg.isst.oe270.degree.remote.processing.communication.IGitMessage
...
class RegisterGitRequest : Message(), IGitMessage, IUsernamePasswordMessage {

    override val blurredFields: List<String>
        get() = listOf(usernameKey, passwordKey)

    override val checkedFields: List<String>
        get() = listOf(timestampKey, repositoryKey, usernameKey, passwordKey)
    ...
    override fun validateFieldValues() {
        ...
        try {
            super<IGitMessage>.validateFieldValues()
        } catch (e: Exception) {
            errors.add(e.message!!)
        }
