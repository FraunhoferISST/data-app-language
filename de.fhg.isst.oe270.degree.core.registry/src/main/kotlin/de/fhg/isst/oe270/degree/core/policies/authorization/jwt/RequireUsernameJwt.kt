...
@PolicyAnnotation("RequireUsernameJwt")
class RequireUsernameJwt : EmbeddedPolicyApi {

    val logger = LoggerFactory.getLogger("RequireUsernameJwt")!!
    ...
    override fun acceptPrecondition(policyInput : PolicyInputScope): Boolean {
        ...
        val currentUsername = (ExecutionContext.getInstance().getModule("JWTUserInformation")
                .getContextEntity("username") as ReadOnlyEntity).read()

        if (username != currentUsername) {
            logger.error("Validation failed. Required JWT username is '$username' but " +
