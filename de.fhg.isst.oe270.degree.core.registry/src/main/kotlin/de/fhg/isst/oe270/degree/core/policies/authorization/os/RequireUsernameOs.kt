...
@PolicyAnnotation("RequireUsernameOs")
class RequireUsernameOs : EmbeddedPolicyApi {

    val logger = LoggerFactory.getLogger("RequireUsernameOs")!!
    ...
    override fun acceptPrecondition(policyInput : PolicyInputScope): Boolean {
        ...
        val currentUsername = (ExecutionContext.getInstance().getModule("OsUserInformation")
                .getContextEntity("username") as ReadOnlyEntity).read()

        if (username != currentUsername) {
            logger.error("Validation failed. Required Os username is '$username' but " +
